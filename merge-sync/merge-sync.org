...
block final (x <= f) yields the 'ok' response. Attempting to mark an unknown future block
final (x > f+1) restarts sync on B_x. For x == f+1, the block is simply appended to the
database and f = f+1.
