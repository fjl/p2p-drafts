...
to requests, but cannot request any information from eth2.
...
The eth1 sync will usually take quite a bit of time to complete. While it is syncing, the
the beacon chain advances by t blocks to the latest finalized block b_{F+t}.

The eth1 client signals that it is done by responding with synced(B_{F+t}), The
