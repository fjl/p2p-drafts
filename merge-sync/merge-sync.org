...
block B_f, clients must handle final(B_x) as follows: At , marking a marking a previously
seen block final (x <= f) yields the 'ok' response. For x == f+1, the block is simply
appended to the database and f = f+1. Attempting to mark an unknown future block final (x
> f+1) restarts sync on B_x.
